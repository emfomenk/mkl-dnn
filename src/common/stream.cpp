...
status_t mkldnn_stream_create(stream_t **stream, stream_kind_t stream_kind) {
    bool args_ok = stream != nullptr && utils::one_of(stream_kind,
            stream_kind::eager, stream_kind::lazy, stream_kind::eager_dummy);
    if (!args_ok)
        return invalid_arguments;
    ...
    case stream_kind::eager_dummy: s = new stream_eager_dummy_t; break;
...
status_t mkldnn_stream_submit(stream_t *stream, size_t n,
    ...
    // A fast path for an eager dummy stream
    if (stream->kind() == stream_kind::eager_dummy) {
